    ...
    *,
    ...
    def dfs(
        ...
        *,
        ...
        # ── Process normal non-chain transitions ──────────────────────────
        for tr in sorted(normal_transitions, key=lambda tr: (tr.dst, tr.action_code, tr.condition, tr.intent)):
            ...
            # FIX 3: Rename loop variable to 'n' to avoid shadowing the outer
            # 'node' parameter. path_nodes already contains the current node
            # (appended by the caller before recursing), so this set is correct.
    ...
    def _dfs_repeat_chain(
        ...
        for dst, action_code, step_condition, trans_list in grouped_transitions:
            ...
            # FIX 4 (root cause): Build new_path BEFORE the condition check so
            # that nodes visited at earlier chain levels are visible to YES/NO
            # conditions at later levels.  Also rename loop var to 'n' to avoid
            # shadowing any outer 'node' name.
            new_path = path_nodes + [dst]
            allowed, step_condition = _evaluate_step_condition(
            ...
            # Max mode keeps ordered repeat-chain traversal, but each repeat
            # level may also exit into normal transitions from the destination.
            # consumed_chains prevents that exit branch from restarting the
            # same repeat chain, while the recursive call below still advances
            # to the next ordered "lần N" level.
            if dst in graph:
                dfs(
                    dst,
                    new_steps,
            ...
            if not is_last_in_chain:
                _dfs_repeat_chain(
                    ...
                    path_nodes=new_path,          # FIX 4 cont.: pass new_path forward
                    source_rows=new_source_rows,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate max E2E Test Cases without repeat-chain grouping."
    )
